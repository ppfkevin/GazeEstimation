...
def Gaze_vector(labels):
    ...
    vector_left = [tmp_list[-10] - tmp_list[-6], tmp_list[-9] - tmp_list[-5]]
    vector_right = [tmp_list[-8] - tmp_list[-3], tmp_list[-7] - tmp_list[-2]]
    if vector_right == [0, 0]:
        gaze_vector = vector_left
    elif vector_left == [0, 0]:
        gaze_vector = vector_right
    else:
        gaze_vector = np.array([vector_left, vector_right]).mean(axis=0)
    std = np.sqrt(gaze_vector[0]**2 + gaze_vector[1]**2)
    gaze_vector = [gaze_vector[0]/std, gaze_vector[1]/std]
    return gaze_vector
